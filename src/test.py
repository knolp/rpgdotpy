...
CHARACTER_TILES = {'stone': 0,
                   'floor': 1,
                   'wall': 0}


class Generator():
    def __init__(self, width=96, height=37, max_rooms=15, min_room_xy=5,
                 max_room_xy=10, rooms_overlap=True, random_connections=1,
    ...
    def gen_tiles_level(self):
        ...
        for row_num, row in enumerate(self.level):
            ...
            for col_num, col in enumerate(row):
                if col == 'stone':
                    tmp_tiles.append(str(self.tiles['stone']))
                if col == 'floor':
                    tmp_tiles.append(str(self.tiles['floor']))
                if col == 'wall':
                    tmp_tiles.append(str(self.tiles['wall']))
    ...
    def create_door(self, grid):
        ...
        for i in range(width):
            for j in range(height):
                if 1 < self.count_alive_neighbors(grid,i,j) < 4 and grid[j][i] == 1:
    ...
    def get_level(self):
        ...
        raw_map = self.tiles_level.copy()
        x,y = self.create_door(raw_map)
        raw_map[x][y] = 3
        ...
        _ret_list = ["".join([translate[x] for x in sublist]) for sublist in raw_map]
        return {
            "map" : _ret_list[:37],
            "door_pos" : (x,y),
            "raw_map" : raw_map
